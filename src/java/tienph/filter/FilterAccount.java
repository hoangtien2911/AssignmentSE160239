...
public class FilterAccount implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            boolean checkURIAdmin = uri.contains("/AdminController")
                                 || uri.contains("/admin");
            boolean checkURIUser = uri.contains("/user/changeProfile.jsp")
                                   || uri.contains("/user/orderDetail.jsp")
                                   || uri.contains("/user/orderHistory.jsp");                        
            if (checkURIAdmin) {
                AccountDTO acc = (AccountDTO) session.getAttribute("ACCOUNT_ADMIN");                
                if (acc == null) {                    
            ...
            if (checkURIUser) {
                AccountDTO acc = (AccountDTO) session.getAttribute("ACCOUNT_USER");
                if (acc == null) {
                    request.setAttribute("NOTI_MUST_LOGIN", "You must login to view this site.");
